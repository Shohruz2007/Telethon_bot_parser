...
# @bot.on(events.NewMessage(pattern='📃 список не иннициализированных каналов'))
...
async def no_initilized_channels_handler(query_data):
    ...
    # query_decode_data = query_data.data.decode('utf-8')
    query_data_message = query_data.message.message
    if query_data_message == "📃 список не иннициализированных каналов":
        return
    ...
    if "назад" in query_data_message:
        ...
        try:
            ...
            Channels_List.Current_list = "Список не иннициализированных каналов"
            print("GOTTEN CHANNEL -->", Channels_List.Editing_channel)
            await bot.send_message(
                await get_chat_id(),
                f"Канал:\n    {channel['channel_name']}",
...
async def channel_main_action(channel_action):
    ...
    if "Файл с значениями канала по умолчению" in action_message:
    ...
    if "Список не иннициализированных каналов" in action_message:
    ...
    if (
        ...
        if not file_update_err is None:
            await bot.send_message(
                await get_chat_id(),
                f"Ошибка при записовании изменения в папке\nERROR:{file_update_err}",
            )
...
async def message_action(request_message):
    ...
    if "send" in query_message_text.lower():
        ...
        if check_message_is_full(found_signals):
            ...
            try:
                await send_message_to_channel(found_signals)
