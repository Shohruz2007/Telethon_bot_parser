"""
Файл с основнимы данными которые могут быть изменени в ходе работы бота
"""
...
class Utility:
    ...
    State = -1 # нинещный статус или ход на катором бот работает 
    Paginator_value = 20 # количество каклов для показа в лизте каналов 


    Local_file_names = [{'bdg':['info','bd_def','bd_sig','instrument','tendency','entrance_point','stop_loss','leverage', 'take_profit']},{'out_message':['status']}] 
    Local_files_list = {
        'out_message':[
                        {
    ...
    Global_files_list = {
                        'bdg':
                        {
                            ...
                            'bdg_def': {'commands':['Общий файл с значениями по умолчению'],
                        ...
                        'templates':
                        {
                            ...
                            'templates_comment': {'commands':['Файл с шаблоном для коментария к сигналу'],'file_path':'templates/templates_comment.txt'},
                        ...
                        'out_message':{
                            ...
                            'signal_all': {'commands':['Файл с приоритетами каналов'], 'file_path':'out_message/config_out_message.csv'},
                        },
    ...
    Commands_list = [
                    ...
                    {'check_commands':['/not_conf_channels', 'список не иннициализированных каналов',],'run_func':'get_no_configured_channels_list','states':[10],'send_message_obj':False},
                    {'check_commands':['/not_configured_channels', 'Cписок не иннициализированных каналов',],'run_func':'get_no_configured_channels_list','states':[10],'send_message_obj':False},
    ...
    Signals_template = ('ID','Channel_name','Group','Status','Channel_link','Admin','Rate', 'Confidence_rate', 'Signal_rate', 'Signals', 'Workspace', 'Instrument', 'Tendency', 'Leverage', 'Margin_type', 'Entrance_point','Stop_loss','Take_profit')
    ...
    Channel_status_template = ('Priority','Status','Group','ID','Channel_name','Test_mode','Origin_message_send','Photo_send','Comment_send',)


    Min_signal_amount = 2   # минимальное количество сигналов при нахождение который сообщение будет считаться сигналом
    Common_tendency_signal_types = ('LONG', 'SHORT', 'ЛОНГ', 'ШОРТ') # Основные типы тенденсии
    Assumed_file_types = ('csv','txt','jpg') # Типы файлов с которыми бот может работать 
    Message_check_symbol = ' :  ' # символ в боте для разделение сигнала с его значением 
    Signal_numer_symbol = '<num>' # символ в сигнал файлах для обозночение чисел
    Channel_signal_send_order = 0 
    Is_stats_update = True # Нужно ли авто обновление некоторых данных как общое количество каналов, количество добавленных каналов и так далее(не стоит отключат)
    Stats_update_time = 86400 # Время авто обновление данных
    Full_channels_amount = 0 
    Check_days = 3 # Время для проверки на отсуствие сигнала от каналов(к примеру 3 то если в течении 3 дней от канала не было сигналов то придёт оповещение)
