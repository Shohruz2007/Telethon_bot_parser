...
async def send_message_to_channel(signals_list, notify=False, origin_message=None):
            ...
            if send_channels_list:
                for channel_status_data in send_channels_list:
                    try:
                        ...
                        #TODO CHANNEL STATUS CHECK AND SIGNAL REPLACER
                        channel_status = channel_status_data['status']
                        ...
                        try:
                            ...
                            try:
                                (sending_message,tendency_type,) = await render_message_from_template(
                                    Utility.Global_files_list["templates"]["templates_signal"]["file_path"],
                                    signals_list,
                                )
                            except:
                        ...
                        try:
                            ...
                            try:
                                if (
                                    channel_status_data["comment_send"].lower() == "true"
                                ):
...
async def resend_edit_message(
    ...
    if not is_send:
        ...
        await bot.send_message(
            await get_chat_id(),
            message=origin_message.replace("Не полное информация", ""),
            parse_mode="html",
            link_preview=False,
        )
