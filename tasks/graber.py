...
from tasks.functions import check_message_is_full, resend_edit_message, send_message_to_channel
...
async def check_message_in_signals(
    ...
    if len(check_important_signal_list) == len(def_found_signals):
        if check_message_is_full(found_signals_values):
            ...
            message_send_try = await send_message_to_channel(
                found_signals_values,
            ...
            # message_send_try = await send_message_to_channel(found_signals_values, origin_message=message)
